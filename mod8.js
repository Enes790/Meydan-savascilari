// ========== mod7.js (BUZUL ÇAĞI) - GÜNCELLEMELER ==========
// - Siperler üst üste binebilir (mesafe kontrolü yok).
// - Buz Boğası: 6000 can, 3 saniyede bir hasar verir, öfke sırasında dönemez.
// - Heykel knockback: 25 (Buz Botu kadar).
// - Heykel Tıraşı ilk doğduğunda 4 saniyede heykel yapar.
// - Buz Boğası siperi parçalayıp durmadan devam eder, sadece duvar durdurur.
// - Heykelden çıkan büyük buz slime ölünce 2 küçük buz slime çıkarır.
...
(function () {
    ...
    const HEYKEL_INSAA_SURESI = 300;       // normal 5 saniye
    const HEYKEL_ILK_INSAA_SURESI = 240;   // ilk doğduğunda 4 saniye
    ...
    const HEYKEL_ITME_MESAFE = 25;          // Buz Botu kadar
    ...
    // Buz Boğası
    const BOGA_HP = 6000;                  // can düşürüldü
            ...
            for (let i = buzBogalari.length - 1; i >= 0; i--) {
                ...
                if (b.durum === 'takip') {
                    ...
                    if (b.sarjSure <= 0) {
                        ...
                        if (canSeePlayer) {
                            b.ofkeAci = Math.atan2(player.y - b.y, player.x - b.x);
                        } else {
                            b.ofkeAci = b.angle;
                        }
                    }
                } else if (b.durum === 'ofke') {
                    // Öfke sırasında dönüş yok, sadece sabit açıda ilerler
                    b.ofkeSure -= ts;
                    ...
                    for (let j = obstacles.length - 1; j >= 0; j--) {
                        ...
                        if (getDist(b, o) < b.radius + o.radius) {
                            ...
                            // Buz Boğası devam eder, sersemlemez
                        }
                    }
    ...
    window.GAME_EXT.hooks.getExtraEnemies = function () {
        ...
        extras = extras.concat(buyukBuzSlimeLari.filter(b => !b.isDead));
        ...
        extras = extras.concat(buzBogalari.filter(b => !b.isDead));
        return extras;
    };
    ...
    window.draw = function () {
        ...
        // Büyük Buz Slime'ları
        buyukBuzSlimeLari.forEach(b => {
            if (b.isDead) return;
            ctx.save();
            ctx.translate(b.x, b.y);
            ctx.fillStyle = '#e74c3c';
            ctx.fillRect(-20, -b.radius - 10, 40, 4);
            ctx.fillStyle = '#2ecc71';
            ctx.fillRect(-20, -b.radius - 10, 40 * (b.hp / b.maxHp), 4);
            ctx.rotate(b.angle);
        ...
        buzBogalari.forEach(b => {
            ...
            ctx.fillStyle = 'rgba(0,0,0,0.3)';
            ctx.beginPath();
            ctx.ellipse(0, b.radius * 0.6, b.radius * 0.9, b.radius * 0.35, 0, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.moveTo(8, -14);
            ctx.quadraticCurveTo(15, -25, 20, -28);
            ctx.lineTo(16, -14);
            ctx.closePath();
            ctx.fill();
            ctx.beginPath();
            ...
            if (b.durum === 'sarj') {
                ...
                ctx.strokeStyle = `rgba(255, 107, 53, ${0.5 + ilerleme * 0.5})`;
                ctx.lineWidth = 3;
                ctx.beginPath();
                ctx.arc(0, 0, b.radius + 8, -Math.PI / 2, -Math.PI / 2 + ilerleme * Math.PI * 2);
                ctx.stroke();
                ctx.rotate(b.angle);
            }
